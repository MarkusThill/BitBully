...
BitBully uses submodules, so make sure to always clone with them initialized.
If you already cloned earlier without them, run:
...
This installs BitBully in editable mode plus all development tooling
(ruff, mypy, pytest, coverage, commitizen, pre-commit, etc.).

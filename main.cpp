...
void verify()
{
    ...
    for(i=0;i<86892;i+=7) //40
    {
        ...
        if(x==1000 && OPENING[i].m_value != 2) {
            std::cerr << "i=" << i << ", x=" << x  << ":  if(x==1000 && m_pOpeningBook8Ply_2[i].m_value!=2)" << std::endl;
            break;
        }
        else if(x==-1000 && OPENING[i].m_value != 1) {
            std::cerr << "i=" << i << ", x=" << x  << ": else if(x==-1000 && m_pOpeningBook8Ply_2[i].m_value!=1)" << std::endl;
            break;
        }
        else if(x==0 && OPENING[i].m_value != 0) {
            std::cerr << "i=" << i << ", x=" << x  << ": else if(x==0 && m_pOpeningBook8Ply_2[i].m_value!=0)" << std::endl;
            break;
        }
...
int main() {
    Reset(); //entleert das Spielfeld. Die Variablen m_fieldP1 und m_fieldP2 werden auf 0 gesetzt.
    BuchLaden("../m_openingBook8Ply.dat");  //Die Er�ffnungsdatenbank wird direkt zu beginn aus der Datei eingelesen
    BuchLaden2("../data8ply.dat");
    MaxInstance=100;  //Der Standardwert f�r die Schwierigkeitsstufe wird gesetzt. Die Schwierigkeitsstufe ergibt sich aus (m_maxSearchDepth-2)/2

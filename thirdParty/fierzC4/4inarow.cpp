    ...
    for (depth = 1; depth <= remainingmoves; depth += 2) {
        if (theoretic) {
            ...
            if (searchwin) {
                ...
                sprintf(status, "depth %i/%i move %i value %i nodes %lu time %.2f hash %.1f %.1fkN/s", depth, maxdepth,
                        column, value, nodes, t, (float) (100 * hashhits) / ((float) (hashhits + hashmisses)), knodes);

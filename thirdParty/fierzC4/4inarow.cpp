...
// 2002 Martin Fierz
//
// implemented:		- bitboard architecture using 64-bit integers (seems
// reasonably fast)
//					- hashtable
...
// benchmarks:	- with 4M entries in hashtable, 4inarowBB solves 4inarow in 5.4
// hours.
...
str[1024], int *playnow, int info, int unused, struct CBmove *move); typedef INT
(WINAPI* PROC3)(char str[255]); typedef unsigned int (WINAPI* PROC4)(void);
typedef unsigned int (WINAPI* PROC7)(int key);
typedef INT (WINAPI* PROC6)(int *board, int color, int from, int to, struct
CBmove *move); typedef INT (WINAPI* COMMANDPROC)(char command[256], char
...
HASHENTRY ht[HASHSIZE_F + HASHITER];
...
void readlearnfile(void) {
  ...
  while (!feof(fp)) {
    ...
    x = fread(&q.red, sizeof(int64), 1, fp);
    x = fread(&value, sizeof(int), 1, fp);
...
int negamax(POSITION *p, int depth, int alpha, int beta) {
  ...
  // hashlookup
  if (depth > 0) {
    if (retrieve(p, depth, &alpha, &beta, &value, &lastbestindex)) return value;
  }
...
*/
  ...
  // threats!!
  if (depth <= 0) {
    if (!isforced) return eval(p);
  }
  isforced = 0;

  // check win using threats; at the same time, order moves to front if they are
  // forced remember the fact that a move is forced in the boolean "isforced"
  if (p->color == YELLOW) {
    for (i = 0; i < n; i++) {
      if (m[i] & redthreat) {
        ordervalues[i] += 1024;
  ...
  while (i < HASHITER) {
        ...
        (int32)(hashkey >> 32))  // super-hotspot: 30% of program!
    {
      ...
      *bestindex = ht[index + i].best;
      ...
      if (ht[index + i].valuetype == LOWER_F) {
      ...
      if (ht[index + i].valuetype == UPPER_F) {
        if (ht[index + i].value <= *alpha) {
          *value = ht[index + i].value;
          return 1;
        }
...
int threatevaluate(int64 yt, int64 rt, int64 dt) {
  ...
  if ((bitcount(playablesquares) - 22) & 1) {
    ...
    if (!(rt & (~threatmask))) {
      ...
      while (tmpthreat) {
        giveup =
            (tmpthreat & (tmpthreat - 1)) ^ tmpthreat;  // least significant bit

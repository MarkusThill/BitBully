...
{
	Empty = new TPicture;
	Empty->LoadFromFile(ExtractFileDir(Application->ExeName)+"\\Images\\empty.bmp");
	Yellow = new TPicture;
	Yellow->LoadFromFile(ExtractFileDir(Application->ExeName)+"\\Images\\yellow.bmp");
	Red = new TPicture;
	...
	BuchLaden();  //Die Er�ffnungsdatenbank wird direkt zu beginn aus der Datei eingelesen
...
void __fastcall TForm1::FormCreate(TObject *Sender)
{
	...
	for(int j=0;j<6;j++)
		for(int i=0;i<7;i++)
		{
			...
			Shape[i][j]->Left = i* left + (width+3)*i+left-(i*3); //Abstand zum linken Rand von GroupBox1
			Shape[i][j]->Top = j* top +(Hoehe+3)*j+3-(j*3);    //Abstand zum oberen Rand von GroupBox1
			...
			ImageSpielfeld[i][j]->Left = i* left + width*i+left+1; //Abstand zum linken Rand von GroupBox1
			ImageSpielfeld[i][j]->Top = j* top +Hoehe*j+4;    //Abstand zum oberen Rand von GroupBox1
			...
			ImageSpielfeld[i][j]->OnMouseDown = ImageClick; //Wenn auf eines der Shapes geklickt wird, soll das Ereignis ShapeClick ausgel�st werden
		}
	StatusBar1->Panels->Items[0]->Width = Form1->Width/2-10; //Panels in der StatusBar sollen in der Mitte geteilt werden
...
{
	if(Button == mbLeft) //Nur ein Klick auf die linke Maustaste soll ausgewertet werden
	{
		int spalte;
		ProgressBar1->Visible=true;
		for(int i=0;i<7;i++)
			for(int j=0;j<6;j++)
				if(Sender==ImageSpielfeld[i][j])//Ermitteln welcher Shape der Sender war(also auf welches Shape geklickt wurde
...
void TForm1::ResetShapes(void)
{
	...
	for(i=0;i<7;i++)
		for(j=0;j<6;j++)
			Shape[i][5-j]->Brush->Color = (TColor)0x004E4E4E;
}
...
void TForm1::GrafikAusgabe(void)
{
	...
	for(i=0;i<7;i++)
		...
		{
			stelle= 41-(i*6+j);
			if(Feld1 & (1i64 << stelle)) //Ist an dieser Stelle das Bit im BitBoard gesetzt???
			{
				if(Modus == 2)
					ImageSpielfeld[i][5-j]->Picture = Red; //5-j, da Grafik oben links anf�ngt, und nicht unten-links
				else
					ImageSpielfeld[i][5-j]->Picture = Yellow; //5-j, da Grafik oben links anf�ngt, und nicht unten-links
			}
...
void TForm1::ZeitmessungStop(void)
{
	...
	StatusBar1->Panels->Items[1]->Text= "Zeit: "+(AnsiString)(h*3600+min*60+sec)+"s"+"   "+msec+ "ms";
}


void __fastcall TForm1::Timer1Timer(TObject *Sender)
{
	/*Nachdem der Computer einen Zug gemacht hat, wird der Timer gestartet und nach einigen MillSec
	wird diese Methode aufgerufen. Hier wird das PAnel wieder aktiviert, sodass der Spieler wieder
	einen Zug machen kann. Diese L�sung mit dem Timer ist notwendig, da das Panel bevor der Berechnung
	deaktiviert wird, sodass der Spieler w�hrend der Berechnung nich ausversehen auf ein Shape klickt
	und dadurch schon den n�chsten Zug gemacht hat. Nach der Berechnung w�rde der Zug trotz der vorherigen
	Deaktivierung des Panels gemacht werden, da erst das Panel wieder aktiviert wird und dann die Botschaften
	verarbeitet werden. Daher wird das Panel erst nach einigen MiiliSec wieder aktiviert, also dann, wenn
	die Botschaften-Warteschleife abgearbeitet ist!!!*/
	Panel1->Enabled=true;
...
{
    /*Wird ausgef�hrt, wenn es sich um ein Mensch-MEnsch SPiel handelt und der Spieler,
     der an der Reihe ist, in eine Spalte geklickt hat. Der Stein wird dann f�r den
     betreffenden Spieler gesetzt und es wird �berpr�ft, ob er mit diesem Zug gewonnen h�tte.
...
void TForm1::ComputerAnziehender(int spalte)
{
    ...
    if(Hoehe[spalte]<6 && Modus==2)
	{
        ...
        if(SpielBeenden2()) //Es wird vor dem Zug des Computers bereits geschaut, ob er gewinnen kann, wodurch dann nachher ein weiterer Zug des Spielers verhindert werden kann
			...
			StatusBar1->Panels->Items[0]->Text="Warten Sie bitte, das Programm rechnet...";
		if(BrettCount()<8)
		{
		...
		{
			Blink(x, SPIELER2);
			StatusBar1->Panels->Items[0]->Text="Sie haben das Spiel leider verloren!!!";
		}
		ZeitmessungStop();
    }
}
